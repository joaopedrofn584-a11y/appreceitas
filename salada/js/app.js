...
function initSecurityNotice() {
  const modal = document.getElementById('security-modal');
  const checkbox = document.getElementById('sec-agree-checkbox');
  const acceptBtn = document.getElementById('sec-btn-accept');
  const sessionIdEl = document.getElementById('sec-session-id');

  // Gerar ou recuperar ID de sessão único
  let sessionId = sessionStorage.getItem('saladafit_session_id');
  if (!sessionId) {
    sessionId = `SES-${Math.floor(100000 + Math.random() * 900000)}-BR`;
    sessionStorage.setItem('saladafit_session_id', sessionId);
  }
  if (sessionIdEl) sessionIdEl.textContent = sessionId;

  // Verificar se o aviso já foi aceito nesta sessão
  const isAccepted = sessionStorage.getItem('saladafit_security_accepted');
  if (isAccepted === 'true' && modal) {
    modal.classList.add('hidden');
  }

  // Listener no checkbox para ativar o botão
  if (checkbox && acceptBtn) {
    checkbox.addEventListener('change', (e) => {
      if (e.target.checked) {
        acceptBtn.removeAttribute('disabled');
        acceptBtn.classList.remove('opacity-50', 'cursor-not-allowed');
        acceptBtn.classList.add('hover:scale-105', 'cursor-pointer');
      } else {
        acceptBtn.setAttribute('disabled', 'true');
        acceptBtn.classList.add('opacity-50', 'cursor-not-allowed');
        acceptBtn.classList.remove('hover:scale-105', 'cursor-pointer');
      }
    });
  }
}
...
function acceptSecurityNotice() {
  ...
  sessionStorage.setItem('saladafit_security_accepted', 'true');
  const modal = document.getElementById('security-modal');
  if (modal) {
...
function selectCategoryTab(categoryName) {
  const tab = document.querySelector(`.cat-tab[data-category="${categoryName}"]`);
  if (tab) {
...
function getAllRecipesList() {
  ...
  if (searchQuery) {
    list = list.filter(r => 
      ...
      (r.subCategory && r.subCategory.toLowerCase().includes(searchQuery)) ||
  ...
  container.innerHTML = recipes.map(recipe => {
    ...
    return `
      <div class="recipe-card bg-emerald-950/40 border border-emerald-800/50 hover:border-emerald-500/60 rounded-2xl overflow-hidden shadow-lg transition-all duration-300 flex flex-col justify-between">
        <div>
          <div class="relative h-44 overflow-hidden bg-emerald-900/40">
            <img 
              src="${recipe.image}" 
              alt="${recipe.title}" 
              onerror="this.onerror=null; this.src='images/salada2.webp';"
            ...
            <button 
              ...
              class="absolute top-3 left-3 w-8 h-8 rounded-full bg-emerald-950/80 backdrop-blur-md border border-emerald-700/60 flex items-center justify-center text-sm transition-transform hover:scale-110 cursor-pointer"
              title="Favoritar"
            >
              ${isFav ? '❤️' : '🤍'}
            </button>
            ...
            <div class="absolute bottom-2 left-3 flex gap-1.5 flex-wrap">
              ${recipe.subCategory ? `
                <span class="bg-amber-500/90 text-slate-950 font-extrabold text-[10px] px-2 py-0.5 rounded-md shadow-sm">
                  ✨ ${recipe.subCategory}
                </span>
              ` : ''}
...
function openRecipeModal(recipeId) {
  ...
  content.innerHTML = `
    <div class="relative">
      <!-- Header Image -->
      <div class="h-48 w-full rounded-2xl overflow-hidden mb-5 relative bg-emerald-900/40">
        <img 
        ...
        <div class="absolute bottom-3 left-4 right-4 flex items-end justify-between">
          <div>
            ${foundRecipe.subCategory ? `
              <span class="text-[10px] font-mono font-bold text-amber-300 bg-amber-950/90 px-2 py-0.5 rounded border border-amber-800/80 mb-1 inline-block">
                ✨ ${foundRecipe.subCategory.toUpperCase()}
              </span>
            ` : `
          ...
          <button 
            ...
            class="text-xl bg-emerald-900/80 p-2 rounded-full border border-emerald-700/60 cursor-pointer"
          >
            ${isFav ? '❤️' : '🤍'}
          </button>
        </div>
      ...
      <div class="mb-6 p-4 rounded-2xl bg-emerald-900/30 border border-emerald-700/50 flex flex-col sm:flex-row items-center justify-between gap-3">
        <div class="flex items-center gap-2">
          ...
          <div>
            ...
            <span class="text-[10px] text-emerald-400/80">Use o timer durante a receita</span>
          </div>
        </div>
        <div class="flex items-center gap-2">
          ...
          <button id="timer-btn-start" onclick="toggleTimer(15)" class="text-xs bg-emerald-600 hover:bg-emerald-500 text-white font-medium px-3 py-1 rounded-lg transition-colors cursor-pointer">
            Iniciar
          </button>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        ...
        <div>
          ...
          ` : `
            <h4 class="text-xs font-mono uppercase tracking-wider text-emerald-400 mb-3 font-semibold">💡 Destaques da Receita</h4>
            <div class="p-4 rounded-xl bg-emerald-950/40 border border-emerald-800/40 text-xs text-emerald-200 space-y-2">
              <p>• <strong>Coleção:</strong> ${foundRecipe.subCategory || 'Adicionais Fit'}</p>
              <p>• <strong>Conservação / Armazenamento:</strong> Prática e pronta para consumir ou congelar.</p>
            </div>
          `}
        </div>
      </div>
